...
const pagSeguroImport = require('../config/pagSeguroConfig.js');
const Company = require('../models/CompanyModel.js');
const Payment = require('../models/PaymentModel.js');

const pagSeguroConfig = pagSeguroImport('producao');

module.exports = {
    subscription: (req, res, next) => {
        const email = pagSeguroConfig.email;
        const token = pagSeguroConfig.token;
		const urlPreApprovalsEmail = pagSeguroConfig.urlPreApprovalsEmail;
		const urlPreApprovalsCode = pagSeguroConfig.urlPreApprovalsCode;

		Company.findOne({_id: req.companyID}, {subscription: 1, price: 1})
			.then((company) => {
				...
				(company.subscription.status != 'ACTIVE' && company.subscription.status != 'PENDING')){
					...
					const params = {
						...
						preApproval: {
							...
							'amountPerPayment': parseFloat(company.price).toFixed(2), 
							'period': 'Monthly',
							'finalDate': endDate.toISOString(),
							'maxTotalAmount': (parseFloat(company.price)*12).toFixed(2)
						}
					};
					...
					let options = {
						...
						uri: urlPreApprovalsEmail + email + '&token=' + token,
						method: 'POST',
						headers: {
					...
					request(options, function(errPS, responsePS, bodyPS) {
						...
						parse2json(bodyPS, {
							...
							let baseURL = urlPreApprovalsCode;
    ...
    notification: (req, res, next) => {
        ...
        const email = pagSeguroConfig.email;
        const token = pagSeguroConfig.token;
		const urlTransactionsNotifications = pagSeguroConfig.urlTransactionsNotifications;
		const urlPreApprovalsNotifications = pagSeguroConfig.urlPreApprovalsNotifications;
        const notificationType = req.body.notificationType;

        let baseURL = '';
        if (notificationType == 'transaction') {
			
			baseURL = urlTransactionsNotifications;

        } else if (notificationType == 'preApproval') {
			
			baseURL = urlPreApprovalsNotifications;
        }
    ...
    cancel: (req, res, next) => {
        const email = pagSeguroConfig.email;
        const token = pagSeguroConfig.token;
		const urlPreApprovalsCancel = pagSeguroConfig.urlPreApprovalsCancel;
		...
		Company.findOne({_id: req.companyID}, {subscription: 1})
			.then((company) => {
                ...
                if (company.subscription != null && company.subscription.status == 'ACTIVE') {
					...
					let baseURL = urlPreApprovalsCancel;
